...
p1 <- ggplot(data = ne.jpn.sf) + 
    ...
    geom_text(aes(x = 133, y = 42, 
                  label = "Only numbers > 100 are shown."), 
              family = font.base, color = col.tw, size = 4) + 
...
p2 <- ggplot(data = ne.jpn.sf) + 
    geom_sf(mapping = aes(), size = 0.05, fill = col.fg, color = "gray50") + 
    geom_sf(mapping = aes(size = Rus.ratio), data = pop.jpn.sf, 
            color = col.el, alpha = 0.5) + 
    lims(x = c(128, 145), y = c(31, 45)) + 
    geom_sf_text(mapping = aes(label = Rus.ratio), 
                 data = pop.jpn.sf %>% filter(Rus.ratio > 5) %>% 
                     mutate(Rus.ratio = round(Rus.ratio, 0)), 
                 color = col.tw, cex = 2, family = font.base) + 
    geom_text(aes(x = 133, y = 42, 
                  label = "Only ratios > 5% are shown."), 
              family = font.base, color = col.tw, size = 4) + 
    labs(x = NULL, y = NULL, title = NULL) + 
    guides(size = guide_legend(title = "Pop Ratio (%)")) + 
    theme(
        title = element_text(color = col.tw, size = 20), 
        legend.position = c(0.85, 0.15), 
        legend.title = element_text(size = 10), 
...
p1 + p2
ggsave("fig/russians.png", width = 9, height = 5.5, dpi = 300)
```
