...
ne.jpn.sf <-
    ...
    rmapshaper::ms_filter_islands(min_area = 1e10) %>% 
    st_transform(crs = "+proj=moll")

ne.sf <- rnaturalearth::ne_countries(scale = 10, returnclass = "sf") %>% 
    st_transform(crs = "+proj=moll")
```
...
p <- ggplot() + 
    geom_sf(data = ne.sf %>% select(name), 
            fill = col.fg, alpha = 1, 
    ...
    geom_sf(data = ne.jpn.sf %>% select(name) %>% summarise(), 
            fill = col.bmc.pink, 
            color = col.bg, 
            alpha = 1, size = 0.1) + 
    ...
    theme(
        ...
        plot.caption = element_text(color = col.sage.gray, size = 6), 
        ...
        panel.grid.major = element_line(color = col.pr.black, size = 0.05),
...
ggsave("fig/Globe.jpg", width = 6, height = 3.5)
```
